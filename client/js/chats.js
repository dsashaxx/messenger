...
socket.on("group_created", async function(group) {
  console.log("Добавлена новая группа:", group.name);
  socket.emit("join_chat", group.id);

  await loadChats();
});

socket.on("chat_created", async function(chat) {
  console.log("Добавлен новый личный чат:", chat.id);
  socket.emit("join_chat", chat.id);

  await loadChats();
});
socket.on('removed_from_group', async function(data) {
  const removedChatId = Number(data.chatId);
  ...
  delete unreadCounts[removedChatId];

  if (Number(currentChatId) === removedChatId) {
    currentChatId = null;
...
socket.on('avatar_updated', async function(data) {
  console.log('Аватар обновлён:', data.username);

  if (Number(data.userId) === Number(me.id)) {
  ...
  if (currentChatType === 'group' && currentChatId) {
    const membersModal =
      document.getElementById('members-modal');

    if (
      membersModal &&
      membersModal.classList.contains('open')
    ) {
      await loadMembers();
    }
  }
